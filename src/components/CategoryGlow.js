...
export default class CategoryGlow extends Component {
  render() {
    return (
      <React.Fragment>
        <Helmet>
          ...
          <div className='px-md-5 pb-2 mx-auto bring-to-front mt-n5 mt-md-n6 bg-contrast'>
            <div className='flex-containerz row'>
              <div className='text-center text-sm-left py-3 col-12 bg-contrast rounded'>
                <h3 className='text-brand'>Skin Glow</h3>
                <span>Improve your skin glow with these amazing products</span>
              </div>
              <ProductConsumer>
                {(value) => {
                  return value.products
                    .filter((products) => {
                      return products.concern === 'skinglow'
                    })
                    .map((product) => {
